...
def customer_tickets(conn, customer_id):
    ...
    query = '''SELECT f.title, s.screen, t.price FROM films f
...
def screening_sales(conn):
    ...
    query = '''SELECT s.screening_id, f.title, COUNT(t.ticket_id) AS tickets_sold FROM films f
     LEFT JOIN screenings s ON f.film_id = s.film_id
     LEFT JOIN tickets t ON s.screening_id = t.screening_id 
...
def top_customers_by_spend(conn, limit):
    ...
    query = '''SELECT c.customer_name, SUM(t.price) AS total_spent FROM customers c
